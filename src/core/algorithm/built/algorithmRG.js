...
function userRecommendedFilm(movies) {
    var max = movies[0];
...
// Test
var best_ph = userRecommendedFilm([ph_ib, ph_du]);
console.log(best_ph);
var best_ben = userRecommendedFilm([ben_ib]);
var best_thain = userRecommendedFilm([thain_ib]);
var best_bast = userRecommendedFilm([bast_ib]);
...
/*
function trier(bests: MovieRatedByUser[]): any {
  let result = [bests.length];
  const matchScoreMap = new Map();
  for(let i=0; i<bests.length; i++) {
    matchScoreMap.set(moviesMatchBetweenTwoUsers(best_ph, bests[i]), bests[i].user);
  }
  let mapDesc = new Map([...matchScoreMap.entries()].sort((a, b) => a[0] - b[0]));
  return mapDesc;
}
*/
